...
function Get-RelativePath([string]$root, [string]$full) {
  ...
  $resolvedFile = Resolve-Path -LiteralPath $full
  $rootPath = $resolvedRoot.ProviderPath
  $filePath = $resolvedFile.ProviderPath
  if (-not $rootPath) { $rootPath = $resolvedRoot.Path }
  if (-not $filePath) { $filePath = $resolvedFile.Path }
  ...
  if ($method) {
    return [System.IO.Path]::GetRelativePath($rootPath, $filePath)
  }
  ...
  $rootUri = New-Object System.Uri -ArgumentList $rootPath, [System.UriKind]::Absolute
  $fileUri = New-Object System.Uri -ArgumentList $filePath, [System.UriKind]::Absolute
