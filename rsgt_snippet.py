...
try:
    import motivation_system as motivation_system
...
from rsia_rcf_bridge import RSIARCFBridge
...
class AutonomousValueFormationSystem:
    """Autonomous Value Formation System"""
...
# --------------------------------------------------------------------------- #
# RSIA ⇄ RCF Bridge Integration Helpers                                       #
# --------------------------------------------------------------------------- #

def _normalized_metric(value: Optional[float], default: float = 0.5) -> float:
    """Clamp metrics retrieved from analysis_result into [0, 1]."""
    if value is None:
        return default
    try:
...
def _ethic_coordinate(value: Optional[float]) -> float:
    """Map [0, 1] metric into the [-1, 1] ethical manifold range."""
    return float(np.clip(2.0 * _normalized_metric(value) - 1.0, -1.0, 1.0))


def build_rcf_bridge_snapshot(analysis_result: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convert the heterogeneous analysis_result emitted by the RSGT engine into the
    dictionary format expected by RSIARCFBridge.ingest_rsgt_snapshot().
    """
    metrics = analysis_result.get("analysis_metrics", {}) or {}

    final_state = analysis_result.get("final_state")
    if final_state is None:
        final_state = analysis_result.get("input_state")
    if final_state is None:
        vector = [0.5, 0.5, 0.5]
    else:
        vector = list(np.asarray(final_state, dtype=float).flatten())

    ethical_vector = [
        _ethic_coordinate(metrics.get("value_autonomy")),
        _ethic_coordinate(metrics.get("goal_formation")),
        _ethic_coordinate(metrics.get("identity_coherence")),
        _ethic_coordinate(metrics.get("temporal_stability")),
        _ethic_coordinate(metrics.get("ral_coherence")),
    ]

    beliefs = {
        "value_autonomy": _normalized_metric(metrics.get("value_autonomy")),
        "goal_formation": _normalized_metric(metrics.get("goal_formation")),
        "identity_coherence": _normalized_metric(metrics.get("identity_coherence")),
        "information_complexity": _normalized_metric(metrics.get("information_complexity")),
        "temporal_stability": _normalized_metric(metrics.get("temporal_stability")),
        "ral_coherence": _normalized_metric(metrics.get("ral_coherence")),
        "tri_axial_score": _normalized_metric(metrics.get("tri_axial_score")),
    }

    snapshot: Dict[str, Any] = {
        "identity_state": {"vector": vector},
        "ethical_projection": ethical_vector,
        "belief_assignments": beliefs,
    }

    improvement_opportunities = analysis_result.get("improvement_opportunities") or []
    contradictions = []
    for opportunity in improvement_opportunities:
        capability = opportunity.get("capability", "capability")
        intensity = opportunity.get("improvement_potential", 0.5)
        priority = opportunity.get("priority", 0.5)
        contradictions.append(
            {
                "type": "identity",
                "description": f"Capability gap: {capability}",
                "intensity": float(np.clip(intensity + 0.1 * priority, 0.0, 1.0)),
            }
        )
    if contradictions:
...
def run_rcf_bridge_integration(analysis_result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Execute a full RCF grounding cycle using the latest RSGT analysis result and
    print a concise grounding summary.
    """
    try:
        bridge = RSIARCFBridge()
        snapshot = build_rcf_bridge_snapshot(analysis_result)
        bridge.ingest_rsgt_snapshot(snapshot)
        bridge.run_grounding_cycle()
        summary = bridge.grounding_summary()

        print("\n=== RSIA↔RCF Bridge Grounding Summary ===")
        print(f"Grounded: {summary.get('grounded')}")
        print(f"Grounding Score: {summary.get('grounding_score'):.4f}")
        print(f"Coherence Index: {summary.get('coherence_index')}")
        print(f"Ethical Alignment: {summary.get('ethical_alignment')}")
        print(f"Metastability: {summary.get('metastability')}")
        print(f"Integrated ES: {summary.get('integrated_es')}")
        print(f"RAL Coherence Flag: {summary.get('ral_coherence')}")

        return summary
    except Exception as exc:
        print(f"RSIA↔RCF bridge integration failed: {exc}")
        return None
...
def run_enhanced_comprehensive_tests():
    ...
    run_rcf_bridge_integration(analysis_result)
